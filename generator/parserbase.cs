...
namespace HumanParserGenerator
{
        ...
        where TRoot : class, ISyntaxNode, new()
    {
        ...
        public ParseError Error => _currentFrame.Error;
        public bool Success => _currentFrame.Success;
        ...
        public ParserBase<TRoot> Parse(string source)
        {
            ...
            if (_currentFrame.Success && !_scanner.IsConsumed)
                _currentFrame.Error = _scanner.CreateError(() => "Cannot parse remaining data");
        ...
        protected void Terminal(string value, Action<bool> onSuccess)
        {
            Terminal(value);
            onSuccess(Success);
        }
